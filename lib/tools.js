 ...
 * @returns {*}
 */
tools.checkData.check_int = function (d, definet) {
	if (parseInt(d) != d) return definet;
	return d;
}
...
tools.checkCSRF = function (req, method) {
	...
	console.log(reffer);
	console.log(csrf);
	if (reffer.indexOf(config.webDomain) > -1 && req.body.csrf == req.session._csrf) {//带域名并且包含csrf参数
		return true;
	}
